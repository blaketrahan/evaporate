...
f4pt msp;
{
	msp.x = (f4)EasyTab->PosX / (f4)user.screen_width * 2.0f;
	msp.y = (f4)EasyTab->PosY / (f4)user.screen_height * -2.0f + 2.0f;  
	...
	if (COLLECT_MOUSE_PTS)
	{
		...
		s4 q = easytab_history.last_pt;
		s4 num_points_in_history = 64;
		for (s4 i = 0; i < num_points_in_history; i++)
		{
			q--;
			if (q < 0)
			{
			...
			mpts[mp_size].x = (f4)easytab_history.pts[q].x / (f4)user.screen_width * 2.0f;
			mpts[mp_size].y = (f4)easytab_history.pts[q].y / (f4)user.screen_height * -2.0f + 2.0f;
			mp_size++;   

			if (easytab_history.pts[q].time < easytab_history.last_query && !catch_two_more_for_catmull)
			{
				num_points_in_history = i + 4;
				catch_two_more_for_catmull = true;
			}
		}
	}
	easytab_history.last_query = time_physics_curr;
}

	// process_easy_tab = true;
	// for (s4 i = 0; i < num_easy_tab_pt; i++)
	// {
	// 	mpts[mp_size].x = (f4)easy_tab_ptsX[i] / (f4)user.screen_width * 2.0f;
	// 	mpts[mp_size].y = (f4)easy_tab_ptsY[i] / (f4)user.screen_height * -2.0f + 2.0f;  
	// 	mp_size++;
	// }
	// std::cout << mp_size << std::endl;
	// num_easy_tab_pt = 0;
// }
