...
achadwick, mypaint:
The guts of Gimp's app/display/gimpmotionbuffer.c and app/display/gimpdisplayshell-tool-events.c 
are worth a look too. Looks like it's using a FIR filter for derived quantities like velocity, 
in conjunction with the regular Catmull-Rom interpolation above if the Euclidean distance is 
greater than 6 screen pixels.
